...
class TestMainPageQaScooter:
    ...
    def test_questions_important_waitfor_correct_header(self, get_driver_and_page, go_to_questions_important, the_question, question_header, expected_value):
        WebDriverWait(get_driver_and_page, 10).until(EC.element_to_be_clickable((the_question))).click()
        actually_value = WebDriverWait(get_driver_and_page, 10).until(EC.presence_of_element_located((question_header))).text
